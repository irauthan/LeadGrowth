...
public class DashboardService {
    ...
    public DashboardKpis getDashboardData(String userEmail) {
        return getDashboardData(userEmail, null, null, null);
    }

    public DashboardKpis getDashboardData(String userEmail, String period, String startDate, String endDate) {
        ...
        LocalDateTime[] range = parsePeriodRange(period, startDate, endDate);
        ...
        List<Lead> allLeadsRaw = leadRepository.findByWorkspaceIdOrderByCreatedAtDesc(workspaceId);
        List<Task> allTasksRaw = taskRepository.findByWorkspaceIdOrderByCreatedAtDesc(workspaceId);
        List<AdMetrics> metricsRaw = adMetricsRepository.findByWorkspaceIdOrderByDateDesc(workspaceId);
        ...
        List<Lead> allLeads = (period != null && !period.isEmpty()) ? allLeadsRaw.stream()
                .filter(l -> l.getCreatedAt() != null && !l.getCreatedAt().isBefore(rangeStart) && !l.getCreatedAt().isAfter(rangeEnd))
                .collect(Collectors.toList()) : allLeadsRaw;

        List<Task> allTasks = (period != null && !period.isEmpty()) ? allTasksRaw.stream()
                .filter(t -> t.getCreatedAt() != null && !t.getCreatedAt().isBefore(rangeStart) && !t.getCreatedAt().isAfter(rangeEnd))
                .collect(Collectors.toList()) : allTasksRaw;

        List<AdMetrics> metrics = (period != null && !period.isEmpty()) ? metricsRaw.stream()
                .filter(m -> m.getDate() != null && !m.getDate().atStartOfDay().isBefore(rangeStart) && !m.getDate().atStartOfDay().isAfter(rangeEnd))
                .collect(Collectors.toList()) : metricsRaw;
    ...
    public LocalDateTime[] parsePeriodRange(String period, String startDate, String endDate) {
        ...
        switch (period.toLowerCase()) {
            ...
            case "today":
                ...
                end = LocalDate.now().atTime(java.time.LocalTime.MAX);
                break;
            case "weekly":
            case "this week":
            case "this_week":
            ...
            case "custom":
                ...
                if (endDate != null && !endDate.trim().isEmpty()) {
                    try {
                        end = LocalDate.parse(endDate.trim()).atTime(java.time.LocalTime.MAX);
                    } catch (Exception e) {
                        end = LocalDate.now().atTime(java.time.LocalTime.MAX);
                    }
                } else {
